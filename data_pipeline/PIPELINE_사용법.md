...
`data_pipeline/recipe_pipeline.py` — 레시피 수집 3단계를 asyncio 큐로 연결한 스트리밍 파이프라인.
...
각 스크립트는 예전처럼 단독 실행도 가능하다 (`uv run python data_pipeline/collect_recipe_links.py "볶음밥" 20` 등).
...
uv run python data_pipeline/recipe_pipeline.py "볶음밥" 20
```
...
uv run python data_pipeline/recipe_pipeline.py "볶음밥" 20 --start 0 --end 200
uv run python data_pipeline/recipe_pipeline.py "볶음밥" 20 --start 200 --end 500

# 1단계 건너뛰기: 기존 recipe_ids.json의 ID를 큐에 흘려보냄
uv run python data_pipeline/recipe_pipeline.py "볶음밥" 20 --skip-collect

# 2단계까지 건너뛰기: 기존 PDF를 바로 3단계로 전달, 구조화만 실행
uv run python data_pipeline/recipe_pipeline.py "볶음밥" 20 --skip-collect --skip-pdf

# 3단계 LLM 동시 요청 수 늘리기 (기존 PDF가 많이 쌓여있을 때 유용)
uv run python data_pipeline/recipe_pipeline.py "볶음밥" 20 --skip-collect --skip-pdf --llm-workers 3
```
