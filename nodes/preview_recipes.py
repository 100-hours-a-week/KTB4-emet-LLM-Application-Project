...
from schems import RecipeOption, RecipeOptionList
...
## rag_adequacy_check가 적합성으로 걸러낸 rag_options(최대 PREVIEW_TOTAL_COUNT개)에
## LLM 생성분을 채워서 recipe_options로 합쳐 반환한다.
## (예전에는 build_rag_recipe_options가 검색된 레시피 전체를 필터링 없이 recipe_options에
##  직접 넣어서, 적합성 평가 결과가 실제 노출 목록에 반영되지 않는 문제가 있었음)
...
def evaluate_rag_options(
    ...
    ## (compute_missing_ingredients는 rag_adequacy_check에서 selected_recipes에 다시 적용되므로,
